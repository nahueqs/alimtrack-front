...
// Función pura de cálculo (Blindada contra tipos incorrectos)
const recalculateProgreso = (
    ...
    const camposRespondidos = new Set(
        respuestasCampos
            .filter(rc => rc.valor !== null && rc.valor !== undefined && String(rc.valor).trim() !== '')
            .map(rc => rc.idCampo)
    ).size;

    const celdasRespondidas = new Set(
        respuestasTablas
            .filter(rt => rt.valor !== null && rt.valor !== undefined && String(rt.valor).trim() !== '')
