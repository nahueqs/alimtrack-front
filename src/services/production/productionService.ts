...
class ProductionService {
  ...
  async getProduccionByCodigo(codigoProduccion: string): Promise<ProduccionProtectedResponseDTO> {
    ...
    return apiClient.get<ProduccionProtectedResponseDTO>(
  ...
  async getUltimasRespuestas(codigoProduccion: string): Promise<EstadoActualProduccionResponseDTO> {
    if (!codigoProduccion) throw new Error('El código de producción es requerido.');
    return apiClient.get<EstadoActualProduccionResponseDTO>(
      `/producciones/${encodeURIComponent(codigoProduccion)}/ultimas-respuestas`
    );
  }

  async guardarRespuestaCampo(
    codigoProduccion: string,
    idCampo: number,
    data: RespuestaCampoRequestDTO
  ): Promise<RespuestaCampoDTO> {
    if (!codigoProduccion || !idCampo)
      throw new Error('El código de producción y el ID del campo son requeridos.');
    return apiClient.put<RespuestaCampoDTO>(
      `/producciones/${encodeURIComponent(codigoProduccion)}/campos/${idCampo}`,
      data
    );
  }

  async guardarRespuestaCeldaTabla(
    codigoProduccion: string,
    idTabla: number,
    idFila: number,
    idColumna: number,
    data: RespuestaCeldaTablaRequestDTO
  ): Promise<RespuestaTablaDTO> {
    if (!codigoProduccion || !idTabla || !idFila || !idColumna)
      throw new Error('Todos los IDs son requeridos.');
    return apiClient.put<RespuestaTablaDTO>(
      `/producciones/${encodeURIComponent(codigoProduccion)}/tablas/${idTabla}/${idFila}/${idColumna}`,
      data
    );
  }
  ...
  async cambiarEstado(
    ...
    return apiClient.put<void>(
      `/producciones/${encodeURIComponent(codigoProduccion)}/cambiar-estado`,
      data
    );
  }
  ...
  async updateProduction(
    ...
    if (!codigoProduccion || !productionData)
      throw new Error('El código de producción y los datos son requeridos.');
    return apiClient.put<ProduccionProtectedResponseDTO>(
      `/producciones/${encodeURIComponent(codigoProduccion)}`,
      productionData
    );
  }
