...
import type {EstadoActualProduccionResponseDTO} from '@/pages/Public/DetalleProduccionPublic/types/Productions.ts';
...
export const exportProductionAsPdf = async (
    ...
    try {
        ...
        const produccionData = [
            ...
            // CORRECCIÓN: Añadimos la fecha de finalización con manejo de nulos
            ['Fecha de Finalización', estadoActual.produccion.fechaFin ? new Date(estadoActual.produccion.fechaFin).toLocaleString('es-AR') : 'En curso'],
        ];
        autoTable(doc, {
            startY: yPos,
            body: produccionData,
        ...
        const metadata = estadoActual.estructura.versionRecetaMetadata;
        const versionData = [
        ...
        for (const seccion of estadoActual.estructura.estructura) {
            ...
            const camposSimplesData = seccion.camposSimples.map(c => [c.nombre, estadoActual.respuestasCampos[c.id] || '']);
            ...
            for (const grupo of seccion.gruposCampos) {
                ...
                const camposGrupoData = grupo.campos.map(c => [c.nombre, estadoActual.respuestasCampos[c.id] || '']);
                autoTable(doc, {
                    startY: yPos,
                    head: [['Campo', 'Valor']],
            ...
            for (const tabla of seccion.tablas) {
                ...
                const respuestasTabla = estadoActual.respuestasTablas.find(rt => rt.idTabla === tabla.id);

                const body = tabla.filas?.map(fila => {
                    const rowData: string[] = [fila.nombre];
                    tabla.columnas?.forEach(col => {
                        const celda = respuestasTabla?.celdas.find(c => c.idFila === fila.id && c.idColumna === col.id);
