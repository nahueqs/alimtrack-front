...
interface NuevaProduccionFormProps {
  ...
  recipeVersions?: VersionRecetaMetadataResponseDTO[]; // Make it optional
...
export const NuevaProduccionForm: React.FC<NuevaProduccionFormProps> = ({
  ...
  const [selectedVersion, setSelectedVersion] = useState<VersionRecetaMetadataResponseDTO | null>(
    null
  );
  ...
  const renderFieldComponent = (field: FormFieldConfig) => {
    switch (field.type) {
      case 'text':
        return <Input placeholder={field.placeholder} prefix={field.prefix} {...field.props} />;
      case 'textarea':
        ...
        {formFields.map((field: FormFieldConfig) => (
          <React.Fragment key={field.name}>
            <Form.Item
              ...
              rules={field.rules}
              validateStatus={
                field.name === 'codigoVersionReceta' && hasVersionsError ? 'error' : undefined
              }
              help={
                field.name === 'codigoVersionReceta' && hasVersionsError
